...
export function Navbar() {
  ...
  return (
    <>
      ...
      {/* Mobile: backdrop */}
      {mobileOpen && (
        <div className="bud-mobile-nav-backdrop" onClick={() => setMobileOpen(false)} />
      )}

      {/* Mobile: floating panel */}
      <div className={`bud-mobile-nav-panel${mobileOpen ? ' open' : ''}`}>
        <div className="bud-nav-items">
          {navItems.map((item) => {
            const isActive = item.path === '/'
              ? location.pathname === '/'
        ...
        <div className="bud-mobile-nav-profile-row">
          <div className="bud-nav-avatar">{getInitials(user?.display_name)}</div>
          <span className="bud-nav-profile-name" style={{ flex: 1, minWidth: 0 }}>{user?.display_name ?? '—'}</span>
          <button
            className="bud-nav-signout-btn"
            style={{ opacity: 1 }}
            aria-label="Sign out"
      ...
      <button
        className="bud-mobile-nav-toggle"
